...
app.post('/api/favorite', async (req, res) => {
  try {
    const data = await client.query(`INSERT into favorites (park_code, owner_id)
    VALUES ($1, $2)
    RETURNING *`, [req.body.favorite, req.userId]);
...
app.get('/api/favorites', async (req, res) => {
  try {
    const data = await client.query(`SELECT park_code from favorites
